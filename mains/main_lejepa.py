...
def main(cfg):
    ...
    dataset_train = LejepaVisionDataset(dataset= dataset_train, transform= lejepa_train_tf, V= cfg.data.V)
    
    dataset_test = BufferAwareConcatDataset(datasets= dss_test)
    dataset_test = LejepaVisionDataset(dataset= dataset_test, transform= lejepa_test_tf, V= 1)
    ...
    trainer = LejepaTrainer(cfg, model, train_loader, val_loader, criterion= None, optimizer= optimizer,
                            device= device, earlystopping= None, scheduler= None, writer= writer)
