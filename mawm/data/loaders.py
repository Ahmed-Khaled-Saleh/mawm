...
# %% auto 0
__all__ = ['get_num_rollouts', 'MarlGridDataset', 'RolloutObservationDataset', 'RolloutSequenceDataset', 'LejepaVisionDataset',
           'D4RL']
...
class MarlGridDataset(torch.utils.data.Dataset):
    def __init__(self, cfg, train= True, msg_tf= None, transform=None):
        data_path = cfg.data.data_dir
        data_size = cfg.data.get('data_size', None)
        self.seq_len = cfg.data.seq_len
        self.agents = cfg.env.agents
        ...
        if data_size:
            print(f"Limiting dataset to {data_size} samples.")
            num_rollouts = get_num_rollouts(data_size, batch_size=batch_size, seq_len=self.seq_len)
            self.rollouts = self.rollouts[:num_rollouts]
        else:
            print(f"Using all {len(self.rollouts)} rollouts in dataset.")
